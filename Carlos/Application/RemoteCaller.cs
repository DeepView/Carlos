...
namespace Carlos.Application
{
    ...
    public class RemoteCaller
    {
        ...
        private bool mIsMustProtocol;//当URL访问的是应用程序集，则指定是否需要进行协议化访问。
        private Dictionary<string, string> mSchemes;//可用的方案集合。
        private Assembly mAssembly;//用于访问的程序集。
        private static RemoteCaller mRouter = null;//一个RemoteCaller实例。
        private static readonly object mRouterLock = new object();//防止多线程调用CreateInstance造成构造异常的线程锁。
        ...
        public string AppSchemeName
        {
            ...
            set
            {
                ...
                mAppSchemeName = value;
            }
        }
        /// <summary>
        /// 获取或设置当前实例中的Via方法在访问应用程序集的时候，是否遵循协议化访问规则。
        /// </summary>
        public bool IsMustProtocol { get => mIsMustProtocol; set => mIsMustProtocol = value; }
        /// <summary>
        /// 获取或设置当前实例中用于访问或者作为被调用方所在的程序集。
        /// </summary>
        public Assembly Assembly { get => mAssembly; set => mAssembly = value; }
        /// <summary>
        ...
        public object Via(Uri url, out HttpStatusCode httpStatusCode)
        {
            ...
            result = method.Invoke(mRouter, new object[] { url });
            return result;
        }
        /// <summary>
